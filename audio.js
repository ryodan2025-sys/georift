...
function safeT(t){ return Math.max(t, AC.currentTime+0.002); }

function makeOsc(type, freq, startT, dur, gainVal, dest){
  if(!audioReady) return;
  try{
    const t=safeT(startT);
...
function startMenuMusic(){
  ...
  function scheduleBar(){
    ...
    try{
      ...
      const st=safeT(now+0.05);
      const chord=chords[bar%chords.length];
      chord.forEach(f=>{
        const o=AC.createOscillator(), g=AC.createGain();
        o.type='sine'; o.frequency.value=f;
        g.gain.setValueAtTime(0,st); g.gain.linearRampToValueAtTime(0.08,st+0.4);
        g.gain.setValueAtTime(0.08,st+1.6); g.gain.linearRampToValueAtTime(0,st+2.2);
        o.connect(g); g.connect(musicGain); o.start(st); o.stop(st+2.3);
      });
      // Sub-bass
      [{f:chord[0]/2,t:st},{f:chord[0]/2,t:safeT(st+1.0)}].forEach(({f,t})=>{
        const o=AC.createOscillator(), g=AC.createGain();
      ...
      [1,1.25,1.5,2,1.5,1.25].forEach((m,i)=>{
        const at=safeT(st+0.5+i*0.12);
        const o=AC.createOscillator(), g=AC.createGain();
        o.type='square'; o.frequency.value=chord[0]*2*m;
        g.gain.setValueAtTime(0.06,at); g.gain.exponentialRampToValueAtTime(0.0001,at+0.1);
        o.connect(g); g.connect(musicGain); o.start(at); o.stop(at+0.13);
      });
    }catch(e){}
    bar++;
  ...
  function scheduleMeasure(){
    ...
    try{
      ...
      for(let i=0;i<4;i++){
        ...
        const st=safeT(now+i*BEAT);
        const o=AC.createOscillator(), g=AC.createGain();
        o.type='triangle'; o.frequency.value=f;
        g.gain.setValueAtTime(0.22,st); g.gain.exponentialRampToValueAtTime(0.0001,st+BEAT*.8);
        o.connect(g); g.connect(musicGain); o.start(st); o.stop(st+BEAT+0.05);
      }
      for(let i=0;i<8;i++){
        const f=melodyNotes[(bar*8+i)%melodyNotes.length]; if(f===0)continue;
        const st=safeT(now+i*(BEAT/2));
        const o=AC.createOscillator(), g=AC.createGain();
        o.type='square'; o.frequency.value=f;
        g.gain.setValueAtTime(0.08,st); g.gain.exponentialRampToValueAtTime(0.0001,st+BEAT*.4);
        o.connect(g); g.connect(musicGain); o.start(st); o.stop(st+BEAT/2+0.05);
      }
      for(let i=0;i<16;i++){
      ...
      for(let i=0;i<2;i++){
        ...
        o.connect(g); g.connect(musicGain); o.start(st); o.stop(st+0.2);
      }
    }catch(e){}
    bar++;
